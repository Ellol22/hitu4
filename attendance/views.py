...
# @api_view(['POST'])
# @permission_classes([IsAuthenticated])
# def start_code_session_api(request):
#     """بدء جلسة كود 6 أرقام للحضور"""
#     try:
...
#         now = timezone.now()
#         session_started = lecture.code_session_started_at

#         # تحقق إذا الجلسة بدأت بالفعل ولم تمر 15 دقيقة
#         if session_started and (now - session_started < timedelta(minutes=15)):
#             return Response({'error': 'Code session already started and still active for this lecture.'}, status=status.HTTP_400_BAD_REQUEST)

#         # ابدأ جلسة جديدة
#         lecture.is_open_for_attendance = True
#         lecture.code_session_started_at = now
#         lecture.save()

#         # بدء تدوير الكود في خيط منفصل
#         threading.Thread(target=rotate_code, args=(lecture, lecture.course), daemon=True).start()

#         return Response({'status': 'success', 'message': 'Code session started.'}, status=status.HTTP_200_OK)
